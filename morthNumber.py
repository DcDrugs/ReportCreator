"""https://github.com/AlexxIT/MorphNumbers но с небольшими изменениями"""
...
class MorphNumber:
    ...
    def words_after_number(self, number: int, text: str) -> list[str]:
        ...
        for word in text.split(" "):
            ...
            if any(item in w.tag for item in ("VERB", "ADJS")):
                if number > 1:
                    grammemes = { "plur" }
                else:
                    grammemes = { "sing" }
...
def parse_number(value: Union[int, float, str]) -> tuple[int, int, int]:
